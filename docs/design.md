...
`ta-gen validate` (127 checks) proves the source is internally consistent: unique keys
...
events, HR starts only for accepted offers that were not lost, and the four
...
claiming a change later than its own snapshot date, no `updated_at` before the record's own
first business date, and `updated_at` never moving backwards for one requisition across
extracts.

`data/fixtures/invalid/` holds the deliberately invalid extracts the contract asks to be
kept separate: one complete but broken batch per documented violation (duplicate offer key,
a renege that erases its acceptance, an HR event after the as-of date, `updated_at` after
`extracted_at`, and a broken seat identity). `ta-gen fixtures` regenerates them, and
`test_validation.py` asserts that each one trips the check it is meant to trip - a fixture
that failed for some unrelated reason would not prove anything.
...
that no offer version or cycle identifier exists, and that an unchanged requisition row
repeats its `updated_at` while a real change advances it. The story tests cover segment
